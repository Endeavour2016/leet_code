 ...
 * @method: 
 * 方法1：维护一个最大堆，前k个元素先放入堆中，然后遍历剩余的元素，
 * 如果当前元素大于堆顶，则跳过该元素（因为已经至少有k个元素小于该元素），否则将该元素插入到堆中
 * 空间：O(K)
 * 时间：O{(n^2-K)*logK}
 */
...
// 我们知道整个二维数组中 matrix[0][0] 为最小值，matrix[n - 1][n - 1] 为最大值，现在我们将其分别记作 low 和 high
// 可以发现一个性质：任取一个数 mid 满足 low ≤ mid ≤ high，那么矩阵中不大于 mid 的数，肯定全部分布在矩阵的左上角。

// 查找的过程中，对 [low, high] 区间不断划分：
// 如果<=mid_value的元素个数 count < k 时，在右半区间查找，此时更新low为mid+1
